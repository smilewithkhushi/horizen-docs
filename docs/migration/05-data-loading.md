...
In this step all the balances obtained in the previous dump step have been loaded into the vault smart contracts. This operation has been performed by firing a batch of transactions, by an authorized Horizen admin (its address was whitelisted in the vault contracts).
...
The concept of "cumulative hash" is used as a "fingerprint" of the dump data. Assuming we have a list of dump tuples composed by [address,balance], ordered by address, we define it with the following pseudo-code rule:
...
Furthermore, in the [migration check step](./06-migration-check.md), the same hash can be recomputed off-chain by any third-party, and compared with the one stored in the smart contract.
